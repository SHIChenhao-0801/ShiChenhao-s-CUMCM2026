...
- 2026-09-13 已在本仓库启用 `core.longpaths=true`，用于读取较深的沙盒核验路径；未启用时 Git 曾把两项实际存在的历史核验 JSON 误报为删除。新 Windows 克隆如遇相同限制，应在仓库内运行 `git config --local core.longpaths true` 后重新检查状态，不能据读取失败提交删除。

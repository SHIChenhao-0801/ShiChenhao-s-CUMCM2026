"""Read the two A inputs, preserve observations, export SI columns and data figures.

Run from the 2026CUMCM root after the S2/data-skill guard has passed:
    C:\\Python314\\python.exe -B paper_output/code/data/prepare_a_data.py
No raw values are filled, removed, fitted or extrapolated. No PDE is solved.
For Visual Studio review, inspect EXPECTED, raw_arrays, environment, radius,
data_plan and run_record. A CLI run does not set the GUI/human review flags.
"""
...
def main() -> None:
    ...
    # Reopen exported values: errors in unit conversion/export should stop the run.

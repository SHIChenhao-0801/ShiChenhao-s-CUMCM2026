"""Where does the thermal balance identity break? Compare both sides numerically.

Diagnostic only. Prints, for the frozen Q23 model over a short horizon:
  LHS = d/dt sum_i 2 w_i R^2 B_i(0) T_i  approximated by the model's own rhs
  RHS = 2 h (T_inf - T_s)
and the individual terms that make up each side, so the mismatch can be located
instead of guessed.
"""

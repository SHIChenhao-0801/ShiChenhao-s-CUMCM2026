"""Locate the remaining thermal-balance mismatch: derivative vs boundary flux.

Prints, on the same trajectory,
  (a) d/dt H evaluated from the model's own rhs,
  (b) the surface exchange rate 2 h R (T_inf - T_s),
for several times and horizons, with the quadrature resolution varied.  A mismatch
that is independent of the quadrature resolution and of the spatial grid is either
an identity error or a genuine violation; this script separates the two.
"""
...
def main():
    for question, shrink in (("Q23", False), ("Q4", True)):
        ...
        try:
            ...
            # cumulative comparison at the end of the horizon, two quadratures
            end = float(run.end_s)

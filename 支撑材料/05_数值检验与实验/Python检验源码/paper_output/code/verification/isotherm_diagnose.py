"""Diagnose why the isotherm scenario overflows the solver's numerical Jacobian.

Measures, at the initial state and at a few states along the baseline trajectory:
  * which right-hand-side components are largest;
  * the surface sensitivity d(flux)/d(C_s) and d(flux)/d(T_s) under the revised
    boundary versus the frozen baseline boundary;
  * whether any state or Jacobian entry is non-finite.

This is a diagnostic island: it changes nothing and writes nothing outside stdout.
"""
...
import isotherm_activity_closure as iso  # noqa: E402
...
def describe(model, label, state):
    ...
    # Right-hand side sensitivity to the surface moisture and temperature.
    d = 1e-9
...
def main():
    ...
    # Walk a short way with the baseline to get a drier, warmer state, then compare.

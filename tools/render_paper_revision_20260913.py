...
cache=ROOT/'tmp/cache/paper_revision_20260913';render=cache/version
...
# User-configured, project-local isolated LibreOffice runtime, not a desktop install.
...
manifest={'version':version,'docx':source.relative_to(ROOT).as_posix(),'docxSha256':hashlib.sha256(source.read_bytes()).hexdigest(),
          'pdf':pdfout.relative_to(ROOT).as_posix(),'pdfSha256':hashlib.sha256(pdfout.read_bytes()).hexdigest(),
          'pageCount':len(doc),'pages':pages,'renderExitCode':proc.returncode,'libreoffice':str(lo/'soffice.com')}
(QA/(version+'_render_manifest.json')).write_text(json.dumps(manifest,ensure_ascii=False,indent=2),encoding='utf-8')
print(json.dumps({'version':version,'pages':len(doc),'starts':[{'page':p['page'],'text':p['text'][:55]} for p in pages]},ensure_ascii=False))

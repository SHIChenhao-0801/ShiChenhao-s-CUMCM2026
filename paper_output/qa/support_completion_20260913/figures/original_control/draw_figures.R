...
root <- normalizePath("paper_output/qa/support_completion_20260913/figures/original_control", winslash = "/", mustWork = TRUE)
...
write.csv(bounds,file.path(root,"axis_range_checks.csv"),row.names=FALSE,fileEncoding="UTF-8")
...
write.csv(data.frame(metric=c("figure_count","input_csv_count","input_data_rows","figure4_report_delta_s","figure4_report_M_minus_threshold","figure4_max_centre_difference","figure4_saved_endpoint_h","new_pde_runs"),
  value=c(6,14,4627,f4events$delta_time_s[2],f4events$max_C_minus_threshold[2],max(abs(f4$max_C-f4$centre_C)),max(f4$time_h),0)),
  file.path(root,"plot_checks.csv"),row.names=FALSE,fileEncoding="UTF-8")
capture.output(sessionInfo(),file=file.path(root,"R_session_info.txt"))
cat("DONE. Plot data remain unchanged. No PDE integration or DOCX modification performed.\n")

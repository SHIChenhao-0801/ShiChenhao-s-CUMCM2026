...
m=json.loads((OUT/'source_manifest.json').read_text(encoding='utf-8'))
assert m['status']=='READY' and m['source_count']==47
content=r'''# 附录 B 支撑材料与复现说明
...
for entry in m['formal_support_files']:
    path=entry['path']
    group=path.split('/')[0] if '/' in path else '根目录'
    local=path.split('/',1)[1] if '/' in path else path
    groups[group].append(local)
count=0
for group in sorted(groups,key=lambda g:(g!='根目录',g)):
    files=sorted(groups[group])
    content+='\n'+group+'（'+str(len(files))+'份）\n\n| 编号 | 相对文件路径 |\n|---|---|\n'
    for path in files:
        count+=1
        content+=f'| {count} | {path} |\n'
assert count==106
(OUT/'support_content.md').write_text(content,encoding='utf-8')
(OUT/'coverage.txt').write_text('当前支撑材料正式文件106份，全部已按冻结清单核对SHA256和字节数。\n附录源码47份、9917行：当前44份 + 输入预处理/Python六图导出/R绘图3份。\n语言：Python43、PowerShell1、JavaScript1、MATLAB1、R1。\n追加两份Python已非注释AST及编译检查，R原生解析表达式等价。\n最新正文六图媒体均直接匹配现有R图像SHA；14份CSV逐字节匹配真实六图导出包。\n代码完整覆盖当前采用生产模型、全部已保留验证和必要数据/作图处理；不追加历史版本堆叠或Word排版工具。\n05真实受限/失败保留，不声称全部源码跑通。\n尚待实际DOCX源码逐行提取复核，运行verify_docx_source.py后以报告为准。\n',encoding='utf-8')
print(json.dumps({'support_markdown':str(OUT/'support_content.md'),'formal_files':count,'characters':len(content),'group_counts':{k:len(v) for k,v in groups.items()}},ensure_ascii=False))

...
采用 $\boldsymbol j_w=-\rho_dD\nabla C$，并以空间均匀的干骨架密度闭合，便有 $D_tC=\nabla\cdot(D\nabla C)$。固定半径时 $\boldsymbol u=0$，圆柱径向散度给出正文式(3)。初值 $C_0=2.55$ 对应湿基质量分数 $C_0/(1+C_0)=0.7183098592$；干基含水率可以大于 1。若将环境指标解释为空气湿度比，其分母与材料干基含水率不同；正文的 $C_{eq}$ 是由环境指标约定的等效边界量。
...
梯度通量与向外物理通量方向相反。轴心面通量为零；表面 Robin 条件直接给出 $g^T_{N+1/2}=hR(T_\infty-T_N)$、$g^C_{N+1/2}=-\beta R(C_N-C_{eq})$。中心不会计算 $1/x_0$；常系数时其热方程为 $\dot T_0=4k(T_1-T_0)/(BR^2\Delta x^2)$。表面节点就是实际表面，不再附加半单元内阻。
...
以下为实际生产程序的核心算法摘录，省略辅助分支和输入输出，需配合原工程上下文使用。完整程序、逐行来源和运行记录保留在支撑材料。

### D.1 题给物性

来源：drying_core.py：properties，采用题给物性分支。
...
来源：drying_core.py：网格权重、harmonic及Kirchhoff内部面通量。
...
来源：drying_core.py：rhs，表面潜热比例为零的基线。
...
来源：analytic_jacobian.py：jacobian，面偏导按附录B构造。
...
来源：drying_core.py：_solve_case_impl，BDF分段求解及事件后真实积分。
...
来源：q3_model.py：completion，四位小时上取并检查原精度浓度。

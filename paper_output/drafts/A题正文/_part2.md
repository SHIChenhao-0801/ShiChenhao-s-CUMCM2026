...
用途：定义状态变量；自变量：$m_w$、$m_d$；因变量：$C$、$w$；单位：kg/kg；物理含义：$w$ 仅用于经验式。
...
用途：温度梯度与热流；自变量：$\partial T/\partial r$；因变量：$q_r$；单位：W/m²；物理含义：取**向外为正**。
...
用途：含水率梯度与通量；因变量：$j_w$；单位：kg/(m²·s)；物理含义：水由高向低迁移。
...
用途：内部温度演化；自变量：$t$、$r$；因变量：$T$；单位：K/s；物理含义：热容量×升温率=净流入热流。
...
用途：收缩域映射为材料域；自变量：$x=r/R(t)$；单位：无量纲；物理含义：$x$ 恒对应同一材料。
...
用途：表面水分交换；单位：kg/(m²·s)；物理含义：由 $C_s-C_{eq}$ 与 $\beta$ 驱动。
...
用途：常物性与 $D(C)$；自变量：$C$；因变量：$\rho$、$c_p$、$k$、$D$；单位：kg/m³、J/(kg·K)、W/(m·K)、m²/s。
